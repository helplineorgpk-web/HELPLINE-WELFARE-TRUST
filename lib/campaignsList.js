...
function buildPresetsForCampaign(c) {
  if (Array.isArray(c.donationPresets) && c.donationPresets.length > 0) {
    return c.donationPresets
      .map((n) => Number(n))
      .filter((n) => Number.isFinite(n) && n > 0);
  }
  return buildPresets(c.details?.packages);
}
...
export function buildCampaignsListProps(campaignsData) {
  return (campaignsData || []).map((c) => {
    ...
    return {
      ...
      presets: buildPresetsForCampaign(c),
    };
  });
}
